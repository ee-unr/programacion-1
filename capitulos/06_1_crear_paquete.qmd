...
En este capítulo vamos a ver cómo reunir todo lo que aprendimos hasta acá en la creación de un paquete de R. Crear un paquete nos permite agrupar funciones relacionadas, documentarlas correctamente y facilitar su uso tanto para nosotros mismos como para otras personas. Aunque pueda parecer una tarea avanzada, el proceso básico de creación de paquetes es accesible, especialmente gracias a herramientas como el paquete `usethis`.
:::
...
Incluso si estamos dando nuestros primeros pasos en Estadística o Ciencia de Datos, desarrollar un paquete nos ayuda a pensar en cómo organizar el código para que sea claro, mantenible y reutilizable. También nos da una idea de cómo se construye el ecosistema de R, una comunidad que se apoya fuertemente en la creación colaborativa de software. 
...
* **Colaboración**: compartir nuestro paquete con otras personas para que usen o contribuyan a nuestro trabajo.
* **Documentación integrada**: incluir explicaciones detalladas sobre cada función, ejemplos y ayudas.
* **Distribución**: subir nuestro paquete a repositorios públicos para que cualquier usuario de R pueda instalarlo.

Aunque a simple vista pueda parecer una tarea para usuarios avanzados, crear un paquete básico es totalmente accesible para estudiantes que ya tienen nociones de funciones, documentación y buenas prácticas. Aprender esto desde temprano tiene muchos beneficios:

* Nos obliga a **pensar en la estructura** del código.
...
* Y lo más importante: nos muestra que **crear herramientas propias es posible**.
...
Trabajaremos con herramientas del ecosistema `tidyverse`, en particular con el paquete `usethis`, que simplifica muchas de las tareas involucradas en la creación de paquetes. También vamos a usar `devtools` y `roxygen2` para compilar y documentar nuestro paquete de manera eficiente.

Antes de comenzar, necesitamos tener instalados los siguientes paquetes:
...
Crear un paquete no es más que seguir ciertas convenciones para organizar un conjunto de carpetas y archivos, ya que, en definitiva, se trata de crear una carpeta en nuestra computadora que tiene ciertos elementos obligatorios y algunos otros opcionales.

Vamos a cargar el paquete `devtools` que incluye muchos otros paquetes que ayudan a simplificar la tarea de la creación de un nuevo paquete.

```{r, eval=FALSE}
library(devtools)
```

La forma más fácil de crear un paquete es ejecutando la función `create_package("ruta\al\paquete")` en cualquier sesión de RStudio que tengamos abierta. Por ejemplo:
...
Notemos que antes de la definición de la función hemos incluido la documentación bajo el formato *roxygen*. En la [Unidad 3](@sec-factorial) discutimos por qué es importante documentar nuestras funciones. En el contexto de la creación de un paquete, se vuelve aún más importante porque se trata de la información que permite generar las páginas del menú de *Ayuda*. Volvé a leer la [Unidad 3](@sec-factorial) para repasar cómo generar de forma automática la estructura de la documentación con *roxygen* y para qué se usa cada *tag* o etiqueta.

Hay una etiqueta en la documentación que no habíamos empleado antes: `@export`. Se usa para indicar que esta función tiene que estar disponible cuando alguien cargue el paquete con `library()`. Un paquete guarda funciones para que las aprovechen los usuarios y las mismas deben tener esta etiqueta. No obstante, tal vez estas funciones internamente llaman a otras funciones auxiliares que creemos conveniente diseñar y que no hace falta que estén a disposición de los usuarios. Esas funciones internas no llevan la etiqueta `@export`.
...
```r
install.packages("bisiestos.tar.gz", repos = NULL, type = "source")
```
