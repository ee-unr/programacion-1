...
::: {.concepto data-latex=""}
Un **script** es un archivo de texto que contiene una serie de instrucciones de escritas en algún lenguaje de programación. En lugar de escribir y ejecutar los comandos uno por uno en la consola, podemos escribirlos en un script y ejecutarlos cuando sea necesario. Esto nos permite organizar mejor nuestro trabajo y evitar repetir tareas manualmente. 
:::
...
#### Creación y guardado de scripts  

Para crear un nuevo script en RStudio:  
1. Ir a **Archivo > Nuevo archivo > Script R**.  
2. Se abrirá un editor donde podemos escribir código.  
3. Para guardar el archivo, ir a **Archivo > Guardar como...** y elegir una ubicación en la computadora. Es recomendable guardar los scripts en una carpeta organizada dentro de un proyecto de RStudio.  

Los scripts en R tienen la extensión **`.R`** (por ejemplo, `mi_analisis.R`).  

#### Ejecutar código desde un script  

Escribir código en un script no lo ejecuta automáticamente. Para ejecutarlo, podemos:  
- Seleccionar una línea y presionar `Ctrl + Enter` (Windows/Linux) o `Cmd + Enter` (Mac).  
- Seleccionar varias líneas y hacer clic en el botón **Ejecutar** en la parte superior del editor.  
- Ejecutar todo el script con `Ctrl + Shift + S` o desde el menú **Código > Ejecutar todo**.  

#### Uso de comentarios  

Los comentarios permiten agregar notas en el código para explicar qué hace cada parte. Son fundamentales para que nuestro código sea comprensible para nosotros y otras personas. En R, los comentarios comienzan con `#` y no afectan la ejecución del código:  

```r
# Calculamos la suma de dos números
suma <- 2 + 3  # Guardamos el resultado en 'suma'
```
