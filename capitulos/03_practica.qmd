...
- Una vez definida la función, guarde `combinatorio(m, n)` junto con `fact()` en un script llamado `funciones.R`.
...
Los números combinatorios sólo están definidos para `m` $\ge$ `n`:
...
Retomando el ejercicio anterior, el número combinatorio `m` tomado de a `n` **con reposición** se define como: 

$$
\bar{C}(m, n) = {m + n - 1 \choose n} = \frac{(m+n-1)!}{(m-1)!n!}
$$

Cree la función `combinatorio2(m, n, r)` para generalizar el cálculo de números combinatorios, siendo `r` un argumento adicional que toma el valor lógico `TRUE` si el cálculo es con reposición, y `FALSE` en caso contrario. 

Observaciones:

- Use la función `combinatorio(m, n)` para implementar esta función
