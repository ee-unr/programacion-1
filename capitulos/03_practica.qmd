...
- Una vez definida la función `combinatorio(m, n)`, guardarla junto con `fact()` en un script llamado `funciones.R`.
- En un script aparte, cargue las funciones en el ambiente global usando `source()` y ejemplifique las siguientes propiedades de los números combinatorios para `m = 5` y `n = 2`:
...
En base al [Ejercicio @sec-p3e1], cree la función `combinatorio2(m, n, r)` para generalizar el cálculo de números combinatorios, siendo `r` un argumento adicional que toma el valor lógico `TRUE` si el cálculo es con reposición, y `FALSE` en caso contrario. 
