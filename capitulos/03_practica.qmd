...
Escribir un programa en R para la creación de la función `combinatorio(m, n)` que calcula el número combinatorio `m` tomado de a `n` (también llamado *coeficiente binomial*), siendo estos números naturales tales que `m >= n`. Un número combinatorio se define como:

$$
C(m, n) = {m \choose n} = \frac{m!}{(m-n)!n!}
$$

Se debe utilizar la función `fact()` para el cálculo de factoriales tal como fue definida en esta unidad. Una vez definida la función `combinatorio(m, n)`, guárdela junto con `fact()` en un script llamado `funciones.R`. En un script aparte, cargue las funciones en el ambiente global usando `source()` y ejemplifique las siguientes propiedades de los números combinatorios para `m = 5` y `n = 2`:
...
Es importante recordar que los números combinatorios sólo están definidos para `m >= n`:

- Pruebe qué ocurre pasando un valor de `n` mayor que `m`.
- ¿Por qué la función no falla?
- ¿Qué problemas podría generar este comportamiento?
...
En base al **Ejercicio 1**, cree la función `combinatorio2(m, n, r)` para generalizar el cálculo de números combinatorios, siendo `r` un argumento adicional que toma el valor lógico `TRUE` si el cálculo es con reposición, y `FALSE` en caso contrario. 

Observaciones:

- Use la función `combinatorio(m, n)` para implementar esta nueva función.
...
- Si `a` es igual a cero, use la función `stop()` para devolver un error informativo.
...
> un número primo es un número entero, por convención mayor que 1, sólo divisible por sí mismo y por la unidad.

Modifique la función `es_primo(n)` para que siempre devuelva el resultado correcto de acuerdo con esta definición. Si el argumento de entrada no es un entero mayor que 1, la función debe imprimir un `warning` como en estos ejemplos:
